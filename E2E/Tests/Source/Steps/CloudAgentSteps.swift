...
class MyAbility: Ability {
    ...
    var abilityName: String = "delay execution"
    ...
    func setActor(_ actor: TestFramework.Actor) {
        self.actor = actor
    }
...
class CloudAgentSteps: Steps {
    @Step("step defined here")
    var stepDefined = {
        print("ran")
        try await Task.sleep(nanoseconds: 1_000_000_000)
        
        let actor = Actor("Batata").whoCanUse(MyAbility())
        
//        try await actor.using(ability: MyAbility.self, action: "waits").waits1s()
        
        try await actor.perform(
            withAbility: MyAbility.self,
            description: "waits"
        ) { ability in
            try await ability.waits1s()
        }
        
        let something = try await actor.perform(
            withAbility: MyAbility.self,
            description: "returns something"
        ) { ability in
            return ability.returnSomething()
        }
    }
    
    @Step("assertion error")
    var assertionError = {
        assertThat(3, equalTo(4))
    }

    @Step("step parameterized here {}")
    var stepParameterized = { (value: String) in
